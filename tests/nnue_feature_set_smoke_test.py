...
def main() -> int:
    ...
    from nnue_model import (  # noqa: PLC0415
        ...
        FULL_THREATS_FEATURE_COUNT,
        HALFKA_V2_HM_FULL_THREATS_FEATURE_COUNT,
    ...
    threat_index = full_threat_feature_index(
        chess.WHITE,
        chess.E1,
        chess.Piece(chess.KNIGHT, chess.WHITE),
        chess.F3,
    ...
    threatened_features = active_features(threatened, chess.WHITE, FEATURE_SET_HALFKA_V2_HM_FULL_THREATS)
    quiet_features = active_features(quiet, chess.WHITE, FEATURE_SET_HALFKA_V2_HM_FULL_THREATS)
    ...
    if threatened_features != active_features(mirrored, chess.WHITE, FEATURE_SET_HALFKA_V2_HM_FULL_THREATS):
        print("horizontal threat mirroring failed", file=sys.stderr)
        return 1

    black_perspective = chess.Board("4k3/8/5n2/7Q/8/8/8/4K3 b - - 0 1")
    if threatened_features != active_features(black_perspective, chess.BLACK, FEATURE_SET_HALFKA_V2_HM_FULL_THREATS):
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        if threat_binary.feature_set != FEATURE_SET_HALFKA_V2_HM_FULL_THREATS:
            ...
            return 1
        if threat_binary.feature_count != HALFKA_V2_HM_FULL_THREATS_FEATURE_COUNT:
