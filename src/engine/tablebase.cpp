...
bool probeable_unlocked(const Board& board, bool root_probe) {
#ifdef CHESSENGINE_HAS_FATHOM
...
#ifdef CHESSENGINE_HAS_FATHOM
    std::lock_guard lock(g_tablebase_mutex);
    return probeable_unlocked(board, root_probe);
#else
    (void)board;
    (void)root_probe;
    return false;
...
#ifdef CHESSENGINE_HAS_FATHOM
    std::lock_guard lock(g_tablebase_mutex);
    if (!probeable_unlocked(board, false)) {
        return std::nullopt;
    }
...
#ifdef CHESSENGINE_HAS_FATHOM
    std::lock_guard lock(g_tablebase_mutex);
    if (!probeable_unlocked(board, true)) {
        return std::nullopt;
    }
    const FathomPosition position = to_fathom_position(board);
