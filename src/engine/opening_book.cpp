...
static_assert(kPolyglotRandom.size() == 781);

constexpr std::uint64_t read_be64(const unsigned char* data) {
...
constexpr std::uint32_t read_be32(const unsigned char* data) {
...
constexpr std::uint16_t read_be16(const unsigned char* data) {
    return static_cast<std::uint16_t>((static_cast<std::uint16_t>(data[0]) << 8) | data[1]);
}

constexpr int polyglot_piece_index(Piece piece) {
    const int color_offset = color_of(piece) == Color::White ? 1 : 0;
    return static_cast<int>(type_of(piece)) * 2 + color_offset;
}

constexpr PieceType promotion_from_polyglot(int promotion) {
    switch (promotion) {
        case 1:
...
bool contains_allowed(const std::vector<Move>& allowed, const Move& move) {
    if (allowed.empty()) {
        return true;
    }
    return std::any_of(allowed.begin(), allowed.end(), [&](const Move& candidate) {
        return same_move(candidate, move);
    });
}

std::uint32_t move_key(const Move& move) {
    return static_cast<std::uint32_t>(move.from)
        | (static_cast<std::uint32_t>(move.to) << 8)
        | (static_cast<std::uint32_t>(move.promotion) << 16);
}

void add_or_merge(std::vector<ProbeMove>& moves, const ProbeMove& candidate) {
    const auto found = std::find_if(moves.begin(), moves.end(), [&](const ProbeMove& existing) {
        return same_move(existing.move, candidate.move);
    });
    if (found == moves.end()) {
    ...
    found->weight = static_cast<std::uint16_t>(
        std::min<int>(std::numeric_limits<std::uint16_t>::max(), found->weight + candidate.weight)
    );
    found->learn = std::max(found->learn, candidate.learn);
}
...
std::uint64_t polyglot_hash(const Board& board) {
    ...
    for (Square square = 0; square < kBoardSquareCount; ++square) {
        ...
        key ^= kPolyglotRandom[static_cast<std::size_t>(64 * polyglot_piece_index(piece) + square)];
    }

    const std::uint8_t castling = board.castling_rights();
    ...
    const Square en_passant = board.en_passant_square();
    if (is_valid_square(en_passant)) {
        const int direction = board.side_to_move() == Color::White ? -8 : 8;
        bool capturable = false;
        for (int file_delta : {-1, 1}) {
            const int file = file_of(en_passant) + file_delta;
            const int rank = rank_of(en_passant) + (direction / 8);
            if (file < 0 || file >= 8 || rank < 0 || rank >= 8) {
                continue;
            }
            const Piece pawn = board.piece_at(make_square(file, rank));
            if (pawn == make_piece(board.side_to_move(), PieceType::Pawn)) {
                capturable = true;
                break;
            }
        }
        if (capturable) {
            key ^= kPolyglotRandom[772 + static_cast<std::size_t>(file_of(en_passant))];
        }
    }
...
std::optional<Move> decode_polyglot_move(Board& board, std::uint16_t raw_move) {
    const int to_file = raw_move & 7;
    const int to_rank = (raw_move >> 3) & 7;
    const int from_file = (raw_move >> 6) & 7;
    const int from_rank = (raw_move >> 9) & 7;
    const PieceType promotion = promotion_from_polyglot((raw_move >> 12) & 7);
    ...
    if (type_of(moved) == PieceType::King) {
        if (from == make_square(4, 0) && to == make_square(7, 0)) {
            to = make_square(6, 0);
        } else if (from == make_square(4, 0) && to == make_square(0, 0)) {
            to = make_square(2, 0);
        } else if (from == make_square(4, 7) && to == make_square(7, 7)) {
            to = make_square(6, 7);
        } else if (from == make_square(4, 7) && to == make_square(0, 7)) {
            to = make_square(2, 7);
        }
    }
...
bool OpeningBook::load(const std::filesystem::path& path, std::string* error) {
    clear();
    ...
    if (ec) {
        if (error != nullptr) {
            *error = "could not stat opening book: " + ec.message();
        }
        return false;
    }
    if (size == 0 || (size % 16) != 0) {
        if (error != nullptr) {
            *error = "invalid Polyglot book size";
        }
        return false;
    }
    ...
    if (!input) {
        if (error != nullptr) {
            *error = "could not open opening book";
        }
        return false;
    }

    std::vector<Entry> loaded;
    loaded.reserve(static_cast<std::size_t>(size / 16));
    std::array<unsigned char, 16> raw{};
    while (input.read(reinterpret_cast<char*>(raw.data()), static_cast<std::streamsize>(raw.size()))) {
        loaded.push_back(Entry{
            read_be64(raw.data()),
            read_be16(raw.data() + 8),
            read_be16(raw.data() + 10),
            read_be32(raw.data() + 12),
        });
    }
    if (!input.eof()) {
        if (error != nullptr) {
            *error = "failed while reading opening book";
        }
        return false;
    }

    if (!std::is_sorted(loaded.begin(), loaded.end(), [](const Entry& lhs, const Entry& rhs) {
            return lhs.key < rhs.key;
        })) {
        std::sort(loaded.begin(), loaded.end(), [](const Entry& lhs, const Entry& rhs) {
            return lhs.key < rhs.key;
        });
    }
    ...
    rng_state_ ^= static_cast<std::uint64_t>(entries_.size()) + polyglot_hash(Board::start_position());
    return true;
}
...
bool OpeningBook::loaded() const {
    return !entries_.empty();
}

const std::filesystem::path& OpeningBook::path() const {
    return path_;
}

std::size_t OpeningBook::entry_count() const {
    return entries_.size();
}

std::optional<ProbeMove> OpeningBook::probe(
    Board& board,
    ...
    const auto first = std::lower_bound(entries_.begin(), entries_.end(), key, [](const Entry& entry, std::uint64_t value) {
        return entry.key < value;
    });
    const auto last = std::upper_bound(first, entries_.end(), key, [](std::uint64_t value, const Entry& entry) {
        return value < entry.key;
    });

    std::vector<ProbeMove> candidates;
    std::unordered_map<std::uint32_t, bool> seen;
    for (auto iterator = first; iterator != last; ++iterator) {
        if (iterator->weight < minimum_weight) {
            continue;
        }
        std::optional<Move> move = decode_polyglot_move(board, iterator->move);
        if (!move || !contains_allowed(allowed_moves, *move)) {
            continue;
        }

        const std::uint32_t key = move_key(*move);
        if (!seen.emplace(key, true).second) {
            add_or_merge(candidates, ProbeMove{*move, iterator->weight, iterator->learn, iterator->move});
            continue;
        }
        candidates.push_back(ProbeMove{*move, iterator->weight, iterator->learn, iterator->move});
    }
    ...
    if (best_book_move) {
        return *std::max_element(candidates.begin(), candidates.end(), [](const ProbeMove& lhs, const ProbeMove& rhs) {
            if (lhs.weight != rhs.weight) {
                return lhs.weight < rhs.weight;
            }
            if (lhs.learn != rhs.learn) {
                return lhs.learn < rhs.learn;
            }
            return move_key(lhs.move) > move_key(rhs.move);
        });
    }

    std::uint64_t total_weight = 0;
    for (const ProbeMove& move : candidates) {
        total_weight += std::max<std::uint16_t>(1, move.weight);
    }
    std::uint64_t pick = next_random(rng_state_) % total_weight;
    for (const ProbeMove& move : candidates) {
        const std::uint16_t weight = std::max<std::uint16_t>(1, move.weight);
        if (pick < weight) {
            return move;
        }
        pick -= weight;
